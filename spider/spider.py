...
def get_detail_url(queue):
    for v in range(10000):
        ...
        queue.put("http://testedu.com/{id}".format(
            id=v))  # Queue队列的put方法用于向Queue队列中放置元素，由于Queue是先进先出队列，所以先被Put的URL也就会被先get出来。
        print("get detail url {id} end".format(id=v))  # 打印出得到了哪些文章的url
